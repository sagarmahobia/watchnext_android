...
/**
 * Created by SAGAR MAHOBIA on 04-Jul-18. at 20:07
 */

@TvFragmentScope
public class AiringTodayAdapter extends BaseRecyclerAdapter {

    private TvFragmentMvpContract.Presenter presenter;

    @Inject
    AiringTodayAdapter(Picasso picasso, TvFragmentMvpContract.Presenter presenter) {
        super(picasso);
    ...
    public void onBindViewHolder(@NonNull CardViewHolder cardViewHolder, int position) {
        presenter.onBindAiringTodayShowCard(cardViewHolder, position);

        cardViewHolder.itemView.setOnClickListener(view -> {
            presenter.onAiringTodayRecyclerItemClick(position);
        });
    }

    @Override
    public int getItemCount() {
        return presenter.getAiringTodayCardsCount();
    }
}
