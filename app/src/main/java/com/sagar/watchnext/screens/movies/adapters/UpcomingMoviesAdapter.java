...
/**
 * Created by SAGAR MAHOBIA on 04-Jul-18. at 13:00
 */
@MoviesFragmentScope
public class UpcomingMoviesAdapter extends BaseRecyclerAdapter {

    private MoviesFragmentMvpContract.Presenter presenter;
    @Inject
      UpcomingMoviesAdapter(Picasso picasso, MoviesFragmentMvpContract.Presenter presenter) {
        super(picasso);
    ...
    public void onBindViewHolder(@NonNull CardViewHolder cardViewHolder, int position) {
        presenter.onBindUpcomingMovieCard(cardViewHolder, position);

        cardViewHolder.itemView.setOnClickListener(view -> {
            presenter.onUpcomingRecyclerItemClick(position);
        });
    }

    @Override
    public int getItemCount() {
        return presenter.getUpComingCardsCount();
